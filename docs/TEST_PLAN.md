...
- `GameSession` shared-path extraction slices:
  ...
  - If the slice moves the exact CPU tail-fire snapshot billboard path from `SharedParticleSnapshotBillboards.cpp`, require a short `opengl_shared` Charmander tail-fire smoke (visual parity + no upside-down/missing texture regression) even if no new helper contract test is added yet.
