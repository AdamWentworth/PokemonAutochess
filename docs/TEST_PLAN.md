...
32. Shared parity regression check (bench/faint/grid/leech-seed): in `opengl_shared` and `d3d12`, verify the bench strip is visible even when board framing is tall, fainting textured models fade cleanly without mask-cutout/material popping, board grid lines remain readable at shallow camera angles, leech-seed projectile is a lobbed arc (not missing), and no dotted green drain connector fallback line appears when the legacy particle snapshot bridge is enabled.
