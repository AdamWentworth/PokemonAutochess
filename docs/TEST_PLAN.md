...
39. Adventure shared pokeball-model parity check (`opengl_shared`, `d3d12`): during capture attempts, verify shared routes render `assets/models/pokeball.glb` (not only a 2D icon fallback), track throw/floor placement/roll correctly, and if your local `pokeball.glb` contains an `open` clip verify absorb-phase playback opens then reverses to close before floor shake/resolve; compare against `opengl` legacy behavior and note any asset-version mismatch (this checkout's raw `pokeball.glb` currently has zero animation clips).
