...
## 2026-02-07 - Stage 6.2 asset pipeline (cooker + packed store)
- Added PAC_DataCooker tool to validate JSON configs and pack scripts/config into a bundle.
- Implemented PackedAssetStore reader for runtime bundles.
- Script/config loaders now pull from the asset store when a pack is provided.
