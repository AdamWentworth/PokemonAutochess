...
[The MIT licence](../LICENSE) applies to original Pokemon Autochess game code,
scripts, tools, tests, build configuration and text documentation authored by
Adam Wentworth, except for the excluded material below. Preserve the copyright
and permission notice when reusing covered code.
...
| --- | --- | --- |
| Recovered field shaders and CPU reference implementations | `src/game/render/materials/field/` | The entire directory is excluded from the MIT grant. See [field provenance](FIELD_MATERIALS.md). |
| Recovered character, eye, lighting and layered-effect programs | `src/game/render/materials/character/` | The entire directory is excluded from the MIT grant. See [character provenance](CHARACTER_MATERIALS.md). |
...
| Runtime asset payloads | Private `assets/`, `content/phlosion/`, cooked models, textures, animations, audio and environment payloads | Outside the MIT grant, whether restored locally or referenced by tracked manifests. |
...
Preserve applicable dependency notices when distributing them. The MIT licence
on original game code does not make every component of a built game or content
bundle MIT licensed.
