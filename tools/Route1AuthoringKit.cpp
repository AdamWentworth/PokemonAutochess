...
bool exportKit(const std::filesystem::path &output, std::string &error) {
    ...
    Json result{
        {"kind", "phlosion_environment_authoring_kit"}, {"schema_version", 1}, {"source", {{"profile_id", identity.profileId}, {"model_sha256", identity.modelSha256}, {"geometry_sha256", identity.geometrySha256}, {"coordinate_system", identity.coordinateSystem}}}, {"objects", Json::array()}, {"trees", Json::array()}};
    for (const auto &object : environment.layoutObjects()) {
        ...
        const Json transform{{"translation", object.sourceTranslationCm},
                             {"rotation_degrees", object.sourceRotationDegrees},
                             {"scale", object.sourceScale}};
        result["objects"].push_back({{"id", object.stableId},
                                     {"display_name", object.displayName},
                                     {"prefab_asset_id", object.prefabAssetId},
                                     {"category_path", object.categoryPath},
                                     {"bounds_minimum_cm", object.boundsMinimumCm},
                                     {"bounds_maximum_cm", object.boundsMaximumCm},
                                     {"transform", transform},
                                     {"imported_source_binding", {{"target_kind", object.targetKind}, {"logical_name", object.logicalName}, {"record_index", object.recordIndex}, {"expected_source_transform", transform}}}});
    }
    const auto tileSnapshot = [&]() {
        Json tiles = Json::array();
        for (const auto &tile : environment.terrainTiles()) {
            tiles.push_back({{"x", tile.gridX}, {"z", tile.gridZ}, {"height", tile.elevationLevel}, {"surface", tile.surface}, {"shape", tile.shape}, {"occupied", tile.sourceOccupied || tile.authored}});
        }
        return tiles;
    };
    ...
    for (const auto &mesh : canonical.meshes) {
        ...
        Json tree{{"source_mesh_index", mesh.sourceIndex},
                  {"pivot_cm", partition.sourcePivotsCm.front()},
                  {"groups", Json::array()}};
    ...
    if (!stream) {
        error = "Could not write authoring kit: " + output.string();
        return false;
    }
