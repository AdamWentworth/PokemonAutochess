...
bool test_combat_targeting_headless(std::string &outFail) {
    ...
    if (!db.moves.loadConfig(engine::paths::data("config/moves_config.json"), &log) ||
        ...
        outFail = "Failed to load combat targeting test data.";
        return false;
    }
    ...
    for (bool scripted : {false, true}) {
        ...
        for (bool reverseOrder : {false, true}) {
            ...
            if (!run(world, [&](const auto &tick) {
                    ...
                    for (int step = 0; step < 8 && std::any_of(units.begin(), units.end(), [](const auto &unit) { return !unit.pendingDamageActive; }); ++step)
                        tick();
        ...
        }
        {
            ...
            if (!run(world, [&](const auto &tick) {
                    ...
                    tick();
                    if (units[0].pendingDamageActive) {
                        outFail = "Moving unit began an attack. scripted=" + std::to_string(scripted);
                        return false;
                    }
                    ...
                    if (units[0].pendingDamageTargetId != units[2].id) {
                        outFail = "Passing a diagonal enemy during movement stole the first combat target.";
                        return false;
                    }
                    return true;
                })) return false;
        ...
        for (int invalidation = 0; invalidation < 7; ++invalidation) {
            ...
            if (!run(world, [&](const auto &tick) {
                    ...
                    tick();
                    if (attacker.pendingDamageTargetId != originalId) {
                        outFail = "Failed to acquire initial diagonal target.";
                        return false;
                    }
                    ...
                    tick();
                    if (attacker.pendingDamageTargetId != originalId) {
                        outFail = "Retargeted an attack already in progress.";
                        return false;
                    }
                    const auto nextAttack = [&]() {
                        finishAnimation(attacker);
                        for (int step = 0; step < 80 && !attacker.pendingDamageActive; ++step)
                            tick();
                        return attacker.pendingDamageTargetId;
                    };
                    if (nextAttack() != originalId) {
                        outFail = "A closer, weaker newcomer stole a still-attackable combat target. scripted=" + std::to_string(scripted);
                        return false;
                    }
                    ...
                    case 0:
                        ...
                        original->fainting = true;
                        break;
                    case 1:
                        original->captureInProgress = true;
                        break;
                    case 2:
                        original->position = world.gridToWorld(7, 6);
                        break;
                    case 3:
                        original->side = PokemonSide::Player;
                        break;
                    case 4:
                        rules->hidden = originalId;
                        break;
                    case 5:
                        rules->wall = originalId;
                        break;
                    case 6:
                        units.erase(std::remove_if(units.begin(), units.end(), [&](const auto &unit) { return unit.id == originalId; }), units.end());
                        break;
                    }
                    if (nextAttack() != fallbackId) {
                        outFail = "Invalid focus prevented retargeting: reason=" + std::to_string(invalidation) + " scripted=" + std::to_string(scripted);
                        return false;
                    }
                    // Nothing attackable: finish the current cycle, then wait.
                    world.findUnitById(fallbackId)->position = world.gridToWorld(0, 0);
                    if (nextAttack() != -1) {
                        outFail = "Started an attack without an attackable enemy.";
                        return false;
                    }
                    return true;
                })) return false;
