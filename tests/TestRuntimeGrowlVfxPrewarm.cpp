...
bool test_runtime_growl_vfx_prewarm_contract(std::string& outFail) {
    ...
    if (cache.find("__authored_vfx_baked:growl_eid_1076:m:assets/textures/moves/growl/Texture3918.png:bake=tev_lerp:alpha=texture") == cache.end()) {
        outFail = "RuntimeGrowlVfxPrewarm should populate baked growl texture entries in the shared backend texture cache.";
        return false;
    }
    if (cache.find("__authored_vfx_baked:growl_eid_1255:q:assets/textures/moves/growl/Texture3924.png:bake=tev_lerp:alpha=texture") == cache.end()) {
    ...
    if (cache.find("__authored_vfx_baked:growl_eid_1275:q:assets/textures/moves/growl/Texture3924.png:bake=tev_lerp:alpha=texture") == cache.end()) {
    ...
    if (cache.find("__authored_vfx_baked:growl_eid_1284:q:assets/textures/moves/growl/Texture3930.png:bake=tev_lerp:alpha=texture") == cache.end()) {
