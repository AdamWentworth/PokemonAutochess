-- scripts/states/classic/route3_shop.lua
...
local commons = { "metapod", "kakuna", "pikachu", "caterpie", "weedle", "pidgey", "spearow", "mankey" }
...
function get_message()
    return string.format("Route 3 Shop - Time left: %ds", math.max(0, math.floor(time_left)))
end
...
function on_shop_card_click(pokemon, level)
    ...
    card_level = card_level or math.random(8, 11)
...
function on_update(dt)
    ...
    if time_left <= 0.0 then
        ...
        persist_cards()
        pop_state()
        push_combat_state("scripts/states/classic/route3.lua")
    end
end
