-- scripts/states/classic/route2_shop.lua
...
local commons = { "caterpie", "weedle", "pikachu", "nidoran-f", "nidoran-m", "spearow", "mankey", "pidgey", "rattata" }
...
function get_message()
    return string.format("Route 2 Shop - Time left: %ds", math.max(0, math.floor(time_left)))
end
...
function on_shop_card_click(pokemon, level)
    ...
    card_level = card_level or math.random(5, 8)
...
function on_update(dt)
    ...
    if time_left <= 0.0 then
        ...
        persist_cards()
        pop_state()
        push_combat_state("scripts/states/classic/route2.lua")
    end
end
