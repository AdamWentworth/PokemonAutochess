...
void Model::loadGLTF(const std::string& filepath)
{
    ...
    auto loadPrimitiveTexture = [&](int materialIndex)->unsigned int {
        unsigned int tex = 0;

        auto makeWhite = [&]() -> unsigned int {
            unsigned int t = 0;
        ...
        if (materialIndex < 0 || materialIndex >= (int)gltf.materials.size()) {
            std::cout << "[GLTF][TEX] materialIndex=" << materialIndex << " invalid -> white\n";
            return makeWhite();
        }
        ...
        if (mat.pbrMetallicRoughness.baseColorTexture.index >= 0) {
            ...
            std::cout << "[GLTF][TEX] material '" << mat.name << "' has no baseColorTexture -> white\n";
            return makeWhite();
        }

        if (texIndex < 0 || texIndex >= (int)gltf.textures.size()) {
            std::cout << "[GLTF][TEX] material '" << mat.name << "' baseColorTexture=" << texIndex << " invalid -> white\n";
            return makeWhite();
        }
        ...
        if (imgIndex < 0 || imgIndex >= (int)gltf.images.size()) {
            std::cout << "[GLTF][TEX] texture=" << texIndex << " has invalid image source=" << imgIndex << " -> white\n";
            return makeWhite();
        }
        ...
        // Critical for RGB textures whose row stride isn't 4-byte aligned:
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

        GLenum format = (img.component == 4) ? GL_RGBA : GL_RGB;
        GLenum internalFormat = (img.component == 4) ? GL_RGBA8 : GL_RGB8;
        ...
        // Defaults: glTF default wrap is REPEAT (important: avoids edge bleeding / wrong colors)
        ...
        std::cout << "[GLTF][TEX] mat='" << mat.name
                << "' matIndex=" << materialIndex
                << " baseColorTex=" << texIndex
                << " img=" << imgIndex
                << " (" << img.width << "x" << img.height << ", comp=" << img.component << ")"
                << " sampler=" << gt.sampler
    ...
    for (int meshIdx = 0; meshIdx < (int)gltf.meshes.size(); ++meshIdx) {
        ...
        for (int primIdx = 0; primIdx < (int)mesh.primitives.size(); ++primIdx) {
            ...
            unsigned int tex = loadPrimitiveTexture(prim.material);
