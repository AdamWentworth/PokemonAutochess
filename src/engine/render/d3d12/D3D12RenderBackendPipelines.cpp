...
float4 main(PSIn i, bool isFrontFace : SV_IsFrontFace) : SV_TARGET {
  ...
  return float4(outSrgb, outA);
    ...
    constexpr std::size_t kSkinMatrixBytesPerDraw =
        kMaxGpuSkinMatrices * 16u * sizeof(float);
    constexpr std::size_t kMaxSkinningDrawsPerFrame = 1024u;
    constexpr std::size_t kSkinMatrixBufferBytes =
        kSkinMatrixBytesPerDraw * kMaxSkinningDrawsPerFrame;
    ...
    // Reserve an identity matrix at offset 0 for non-skinning draws.
