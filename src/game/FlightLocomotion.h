...
// Goal: add a short "takeoff" and a landing sequence (A->B loop->C) for small fliers (e.g. Pidgey)
// so they don't instantly snap between grounded idle and flying locomotion.
//
// IMPORTANT:
// - This does NOT change gameplay timing (movement/attack logic).
// - This DOES apply a code-driven world-space Y offset (PokemonInstance::visualYOffset) for lift/land visuals.
//   If you don't want code-driven lift, set PokemonInstance::airLiftY = 0 (default).
//
// Renderer note:
// Wherever you build the model matrix for a PokemonInstance, add:
//   renderPos = p.position + glm::vec3(0.0f, p.visualYOffset, 0.0f);
...
static inline bool isDebug(const PokemonInstance& p) {
...
static inline const char* stateName(AirLocomotionState s) {
    switch (s) {
...
static inline void logTransition(const PokemonInstance& p,
...
static inline float computeLandingLoopTargetAnimSec(const PokemonInstance& p,
...
static inline bool hasLandingSequence(const PokemonInstance& p) {
...
static inline float computeVisualYOffset(const PokemonInstance& p,
