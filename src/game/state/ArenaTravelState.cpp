...
ArenaTravelState::~ArenaTravelState() {
    onExit();
}
...
void ArenaTravelState::captureFormation() {
    ...
    auto capture = [&](const auto &units, bool bench) {
        for (const auto &unit : units) {
            ...
            formation_.push_back({unit.id, bench, bench ? world_.travelBenchSlot(unit.position) : -1, {cell.x, cell.y}, unit.rotation});
        }
    };
...
bool ArenaTravelState::loadDestination() {
    ...
    for (auto index : displaced) {
        ...
        if (best.x < 0) {
            error_ = "The destination has no free legal cell for the whole formation.";
            return false;
        }
        slot.cell = best;
...
void ArenaTravelState::update(float dt) {
    ...
    switch (phase_) {
    case Phase::Hold:
        if (elapsed_ >= .65f) enter(Phase::Recall);
        break;
    case Phase::Recall:
        if (elapsed_ >= kRecall) enter(Phase::Cover);
        break;
    case Phase::Cover:
        if (elapsed_ >= kCover) enter(Phase::Load);
        break;
    case Phase::Load:
    ...
    case Phase::Warm:
        ...
        break;
    case Phase::Reveal:
        if (elapsed_ >= kReveal) enter(Phase::Throw);
        break;
    case Phase::Throw:
        if (elapsed_ >= kThrow) enter(Phase::SendOut);
        break;
    case Phase::SendOut:
        if (elapsed_ >= kSend) enter(Phase::Ready);
        break;
    case Phase::Ready:
        if (manager_) {
            manager_->popState();
