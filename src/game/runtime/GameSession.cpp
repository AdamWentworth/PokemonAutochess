        ...
        if (showWorldBackdrop) {
            ...
            if (useProjectedWorldLayout) {
                ...
                const auto appendSharedGrowlWaveVfx = [&]() {
                        ...
                        [&](WorldIndexedBatch& batch,
                            ...
                            for (const auto& src : mesh.vertices) {
                                ...
                                if (quantizeLineAlpha) {
                                    vtx.a = game::runtime::shared_growl::quantizeLineVertexAlpha(
                                        srcAlpha, lineTevK1A, color.a);
                                } else {
                    ...
                    for (const auto& pass : growlSnapshot.drawPasses) {
                        ...
                        const bool drawLinePass =
                            game::runtime::shared_growl::isLinePass(growlSnapshot.config, pass);
                ...
                const auto appendSharedCaptureAttemptModels = [&]() -> bool {
                    ...
                    if (isD3d12Backend) {
                        d3d12CapturePrewarmRequested = (gameWorld->getSelectedItem() == "pokeball");
                        if (!d3d12CapturePrewarmRequested) {
                            const auto ownedItems = gameWorld->listItems();
