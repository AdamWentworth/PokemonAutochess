struct LayeredEffectMaterialState {
...
vec2 nativeLayeredMaterialUv(LayeredEffectMaterialState materialState) {
...
vec4 evaluateNativeLayeredUnlitDisplaced(
    ...
    LayeredEffectMaterialState materialState) {
    ...
    bool layeredCharacterCharacterComposite =
...
float flameHash11(float value) {
    return fract(sin(value * 12.9898) * 43758.5453);
}

float flameHash21(vec2 value) {
    return fract(sin(dot(value, vec2(127.1, 311.7))) * 43758.5453);
}

float flameValueNoise(vec2 point) {
    vec2 cell = floor(point);
    vec2 local = fract(point);
    vec2 blend = local * local * (3.0 - 2.0 * local);
    float a = flameHash21(cell);
    float b = flameHash21(cell + vec2(1.0, 0.0));
    float c = flameHash21(cell + vec2(0.0, 1.0));
    float d = flameHash21(cell + vec2(1.0, 1.0));
    return mix(mix(a, b, blend.x), mix(c, d, blend.x), blend.y);
}

float flameSmoothFlicker(float time, float seed) {
    float point = time * 9.0 + seed * 97.0;
    float cell = floor(point);
    float local = fract(point);
    local = local * local * (3.0 - 2.0 * local);
    return mix(flameHash11(cell), flameHash11(cell + 1.0), local);
}
...
float flameFbm(vec2 point) {
    ...
    for (int octave = 0; octave < 5; ++octave) {
        value += amplitude * flameValueNoise(point);
        point *= 2.02;
        amplitude *= 0.5;
    }
    return value;
...
vec2 flameFbmGradient(vec2 point) {
    const float epsilon = 0.03;
    float x = flameFbm(point + vec2(epsilon, 0.0)) -
              flameFbm(point - vec2(epsilon, 0.0));
    float y = flameFbm(point + vec2(0.0, epsilon)) -
              flameFbm(point - vec2(0.0, epsilon));
    return vec2(x, y) / (2.0 * epsilon);
}

vec2 flameCurl(vec2 point) {
    vec2 gradient = flameFbmGradient(point);
    return vec2(gradient.y, -gradient.x);
}

vec2 flameAdvect(vec2 point, float flowY, float amount) {
    vec2 curl1 = flameCurl(point * 1.30 + vec2(0.0, -flowY * 0.10));
    vec2 curl2 = flameCurl(point * 2.70 + vec2(3.1, -flowY * 0.18));
    return point + (curl1 * 0.65 + curl2 * 0.35) * amount;
}

vec3 flameTonemapSoft(vec3 color) {
    return color / (vec3(1.0) + color);
}

vec2 flameClampUvToRegion(
    vec2 localUv,
    vec4 rectUv,
    LayeredEffectMaterialState materialState) {
...
vec4 sampleEffectAtlas(sampler2D baseColorMap,
                         LayeredEffectMaterialState materialState,
                         vec4 rectUv,
                         vec2 grid,
                         float frames,
    ...
    float speed = coherentPlayback
        ? 1.0
        : mix(0.85, 1.10, flameHash11(seed * 31.7 + 2.3));
    ...
    return texture(
        baseColorMap,
        flameClampUvToRegion(cellLocalUv, rectUv, materialState));
}

vec4 sampleEffectAtlasTopLeft(sampler2D baseColorMap,
                                LayeredEffectMaterialState materialState,
                                vec4 rectUv,
                                vec2 grid,
                                float frames,
...
vec4 evaluateAuthoredFlame(
    ...
    vec2 uv = clamp(
        ...
        vec2(0.0),
        vec2(1.0));
    vec4 baked = sampleEffectAtlasTopLeft(
        baseColorMap,
        materialState,
        materialState.rect0,
        flipbook0.xy,
...
float flameLickBlobs(float x, float y, vec2 advectedPoint, float flowY, float seed) {
    float point = y * 6.6 + flowY * 0.55;
    float segment = floor(point);
    float local = fract(point);
    float center1 = (flameHash11(segment + seed * 31.0) - 0.5) * 0.95 * (1.0 - y);
    float center2 = (flameHash11(segment + seed * 73.0) - 0.5) * 0.95 * (1.0 - y);
    ...
    float breakup = flameFbm(advectedPoint * vec2(7.0, 12.0) + seed * 17.0);
...
vec4 evaluateFlameEffect(
    ...
    vec2 wobble = vec2(
        flameSmoothFlicker(time * 0.9, seed + 0.17),
        flameSmoothFlicker(time * 1.1, seed + 0.73)) - 0.5;
    ...
    if ((fireFlags & 8) != 0) {
        return evaluateAuthoredFlame(baseColorMap, materialState);
    }
    ...
    if (hasFirstTexture) {
        vec4 flipbook0 = materialState.flipbook0;
        flipbook1Sample = sampleEffectAtlas(
            baseColorMap,
            materialState,
            materialState.rect0,
            flipbook0.xy,
        ...
        if (hasSecondTexture) {
            ...
            flipbook2Sample = sampleEffectAtlas(
                baseColorMap,
                materialState,
                materialState.rect1,
                flipbook1.xy,
    ...
    if (hasFirstTexture && !hasSecondTexture) {
        vec4 directSample = sampleEffectAtlas(
            baseColorMap,
            materialState,
            materialState.rect0,
    ...
    float speed = hasSecondTexture ? mix(0.95, 1.10, flameHash11(seed * 19.31)) : 1.0;
    ...
    vec2 advectedPoint = flameAdvect(point * vec2(1.20, 1.0) + seed * 6.0, flowY, 0.25);
    float noise = flameFbm(advectedPoint * vec2(2.7, 4.5) + seed * 11.0);
    ...
    float blobs = flameLickBlobs(x, y, advectedPoint, flowY, seed);
    float body = clamp(smoothstep(0.92, 0.12, distance), 0.0, 1.0);

    float proceduralAlpha = body * (0.60 + 0.55 * blobs);
    float calmFlicker = flameSmoothFlicker(time * 1.2, seed);
    ...
    float segmentRandom = flameHash11(segment + seed * 71.3);
    float segmentRandom2 = flameHash11(segment + seed * 19.7 + 5.0);
    ...
    float warp = flameFbm(
        flameAdvect(vec2(x * 0.85, y * 1.2) + seed * 6.0, flowY, 0.22) *
        vec2(4.5, 7.5)) - 0.5;
    float jag =
